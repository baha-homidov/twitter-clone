...
function TweetTimeline(props) {
  ...
  useEffect(() => {
    if (userInfo && tweetData) {
      // set the retweet state variables
  ...
  function handleLike(e) {
    ...
    if (isLikedByUser) {
      ...
      removeLike(tweetData.tweetId, userInfo.uid);
      setIsLikedByUser(false);
    } else {
      setLikeCount(likeCount + 1);
      likeTweet(tweetData.tweetId, userInfo.uid);
